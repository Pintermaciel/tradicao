...
class DataProcessor:
    def __init__(self, pasta_json):
        ...
        self.api_whatsapp = "http://5.161.106.90:8080/chat/whatsappNumbers/teste-pega-contato"
    ...
    def processar_arquivos(self):
        ...
        for arquivo in Path(self.pasta_json).glob('*.json'):
            try:
                ...
                logging.debug(f"Conteúdo do arquivo {arquivo.name}: {conteudo[:100]}...")  # Log dos primeiros 100 caracteres
                ...
                if conteudo.startswith('{'):
                    ...
                    if 'Venda' in dados:
                        ...
                        logging.info(f"Arquivo {arquivo.name} contém {len(vendas)} vendas (formato nível).")
                        
                        # Processa cada venda da lista
                        for venda in vendas:
                            if 'Venda' in venda:
                                try:
                                    vendas_processadas, vendas_ignoradas = self.processar_dados(
                                        venda['Venda'], todas_vendas, todos_pagamentos, todos_itens,
                                        clientes, vendas_processadas, vendas_ignoradas
                                    )
                                except Exception as e:
                                    logging.error(f"Erro ao processar venda da lista: {str(e)}")
                                    vendas_ignoradas += 1
                    else:
                        logging.warning(f"A chave 'nivel' não encontrada no arquivo {arquivo.name}.")
                else:
                    # Se não é um objeto, tentamos processar como uma lista de vendas
                    try:
                        vendas = json.loads(conteudo)
                        logging.info(f"Arquivo {arquivo.name} contém {len(vendas)} vendas (formato lista).")
                        ...
                        logging.error(f"Erro ao decodificar o arquivo {arquivo.name}: {e}")
                        vendas_ignoradas += 1
                    except Exception as e:
    ...
    def processar_dados(self, dados, todas_vendas, todos_pagamentos, todos_itens, clientes, vendas_processadas, vendas_ignoradas):
        try:
            ...
            if qtdade_itens is None or sub_total is None or total_liquido is None:
                logging.warning(f"Venda com id {id_venda} tem campos obrigatórios faltando.")
                vendas_ignoradas += 1
                return vendas_processadas, vendas_ignoradas
    ...
    def verificar_whatsapp(self, telefone, id_venda):
        if telefone:
            ...
            response = requests.post(self.api_whatsapp, json={"numbers": [telefone]}, headers=headers)
            if response.status_code == 200:
                resultado = response.json()
                # Verifica se o número existe
                if resultado and isinstance(resultado, list) and len(resultado) > 0 and resultado[0].get('exists'):
                    logging.info(f"Número de WhatsApp válido: {telefone} na venda ID {id_venda}.")
                    return True  # Número válido
                else:
                    logging.warning(f"Número de WhatsApp não encontrado: {telefone} na venda ID {id_venda}.")
                    return False  # Número inválido
            else:
                # Lê o corpo da resposta para entender o erro
                erro_corpo = response.text  # ou response.json() se a resposta for JSON
                logging.error(f"Erro ao verificar WhatsApp para {telefone} na venda ID {id_venda}: {response.status_code}, corpo: {erro_corpo}")
                return False  # Número inválido
